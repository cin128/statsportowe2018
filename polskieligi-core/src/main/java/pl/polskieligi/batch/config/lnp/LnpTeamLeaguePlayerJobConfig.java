...
public class LnpTeamLeaguePlayerJobConfig extends AbstractJobConfig<TeamLeague> {
	...
	@Bean
	public JpaPagingItemReader<TeamLeague> lnpTlPlayerUpdateReader(EntityManagerFactory entityManagerFactory){
		return  getUpdateReader(entityManagerFactory);
	}

	@Bean public DefaultItemProcessor<TeamLeague> lnpTlPlayerProcessor(ImportLnpTeamLeaguePlayersLogic importLnpTeamLeaguePlayersLogic) {
